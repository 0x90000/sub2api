...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	windsurfUsageFetcher := service.NewWindsurfUsageFetcher(httpUpstream)
	windsurfAccountProbeService := service.NewWindsurfAccountProbeService(accountRepository, proxyRepository, windsurfUsageFetcher)
	windsurfChatBridge := service.NewWindsurfChatBridge()
	adminService := service.NewAdminService(userRepository, groupRepository, accountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory, windsurfAccountProbeService)
	...
	accountTestService := service.NewAccountTestService(accountRepository, geminiTokenProvider, antigravityGatewayService, windsurfAccountProbeService, windsurfChatBridge, httpUpstream, configConfig, tlsFingerprintProfileService)
	crsSyncService := service.NewCRSSyncService(accountRepository, proxyRepository, oAuthService, openAIOAuthService, geminiOAuthService, configConfig)
	windsurfOAuthService := service.NewWindsurfOAuthService(proxyRepository)
	accountHandler := admin.NewAccountHandler(adminService, oAuthService, openAIOAuthService, windsurfOAuthService, geminiOAuthService, antigravityOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator)
	...
	windsurfOAuthHandler := admin.NewWindsurfOAuthHandler(windsurfOAuthService, adminService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, windsurfOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, paymentHandler)
	...
	windsurfModelCatalogService := service.NewWindsurfModelCatalogService(accountRepository)
	windsurfErrorMapper := service.NewWindsurfErrorMapper()
	windsurfGatewayService := service.ProvideWindsurfGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, configConfig, billingService, billingCacheService, deferredService, windsurfAccountProbeService, windsurfModelCatalogService, windsurfUsageFetcher, modelPricingResolver, channelService, balanceNotifyService, windsurfErrorMapper, windsurfChatBridge)
	windsurfGatewayHandler := handler.NewWindsurfGatewayHandler(windsurfGatewayService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, windsurfGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	tokenRefreshService := service.ProvideTokenRefreshService(accountRepository, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, windsurfOAuthService, compositeTokenCacheInvalidator, schedulerCache, configConfig, tempUnschedCache, privacyClientFactory, proxyRepository, oAuthRefreshAPI)
